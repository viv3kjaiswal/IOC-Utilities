...
set "API_KEY=<Paste API here inside the quotes>"
set "IP_FILE=ips.txt"

echo IP ^| ConfidenceOfAbuse ^| ISP ^| ASN ^| Hostname ^| Country ^| Verdict

for /f "usebackq delims=" %%I in ("%IP_FILE%") do (
    ...
    "  'BL'='Saint Barthelemy'; 'SH'='Saint Helena'; 'KN'='Saint Kitts and Nevis'; 'LC'='Saint Lucia'; 'MF'='Saint Martin';" ^
    ...
    "};" ^
    "try {" ^
    "  $url='https://api.abuseipdb.com/api/v2/check?ipAddress=%%I&maxAgeInDays=90';" ^
    "  $resp=Invoke-RestMethod -Uri $url -Headers @{ 'Key'='%API_KEY%'; 'Accept'='application/json' };" ^
    "  $data=$resp.data;" ^
    "  $hostnameList=if($data.hostnames){$data.hostnames -join ';'}else{'-'};" ^
    "  $countryFull = if($data.countryCode -and $countryMap.ContainsKey($data.countryCode)) { $countryMap[$data.countryCode] } else { 'Unknown' };" ^
    "  $verdict=if($data.abuseConfidenceScore -eq 0){'Clean'}else{'Suspicious'};" ^
    "  Write-Output ('%%I | ' + $data.abuseConfidenceScore + ' | ' + $data.isp + ' | ' + $data.asn + ' | ' + $hostnameList + ' | ' + $countryFull + ' | ' + $verdict);" ^
    "} catch { Write-Output ('%%I | - | - | - | - | - | Error') }" 
)
